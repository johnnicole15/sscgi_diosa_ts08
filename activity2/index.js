...
class Pokemon {
  ...
  attack(opponent) {
    console.log(
      `%c level${this.level}%c ${this.name} attack%c level${opponent.level}%c ${opponent.name}`,'font-size: 10px;','','font-size: 10px;',''
    );
  }
  ...
  receiveDamage(damage) {
    ...
    console.log(
      `lvl ${this.level} ${this.name} received -${damage} Damage and ${this.name} defense negating ${this.defense} Damage: %c${this.hp}%c/${this.maxHp} HP`,'vertical-align: super; color: red;',''
    );
    if (this.hp <= 0) {
  ...
  heal() {
    ...
    if (this.hp == this.maxHp) { // checking if pokemons health is already full, then this heal will not reflect instead it only gives the defense boost of 2.
      ...
      this.hp = this.maxHp;
      console.log(
        `${this.name} heals for full health: %c${this.hp}%c/${
          this.maxHp
        }, and receives 2 defense`,'color: lawngreen;',''
      );
